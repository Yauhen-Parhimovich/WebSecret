		...
		<div className={styles['product-card']}>
			<picture className={styles['product-card__image']}>
				<source
					...
					srcSet={`${data.image.tablet.webp_x1} 1x, ${data.image.tablet.webp_x2} 2x`}
					type="image/webp"
				/>
				<source
					media="(min-width: 578px)"
					srcSet={`${data.image.desktop.webp_x1} 1x, ${data.image.desktop.webp_x2} 2x`}
					type="image/webp"
				/>
				<source
					media="(max-width: 375px)"
					srcSet={`${data.image.mobile.webp_x1} 1x, ${data.image.mobile.webp_x2} 2x`}
					type="image/webp"
				/>
				<source
					media="(min-width: 375px)"
					srcSet={`${data.image.tablet.x1} 1x, ${data.image.tablet.x2} 2x`}
				/>
				<source
					media="(min-width: 578px)"
					srcSet={`${data.image.desktop.x1} 1x, ${data.image.desktop.x2} 2x`}
				/>
				<img
					srcSet={`${data.image.mobile.x1} 1x, ${data.image.mobile.x2} 2x`}
					alt="Product image"/>
			</picture>
			<div className={styles['product-card__content']}>
				...
				{data.in_stock
					? <div>
						<div className={styles['product-card__row']}>
							<p className={styles['product-card__price']}>{data.price} ₽</p>
							{data.is_new && <p className={styles['product-card__new']}>Новое</p>
							}
						</div>
						<div className={styles['product-card__row']}>
							<button className={styles['product-card__button']} type="button">
								В корзину
							</button>
